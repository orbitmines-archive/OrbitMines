...
import com.madblock.api.database.*;
import com.madblock.api.database.tables.TableIPs;
import com.madblock.api.utils.DateUtils;
import com.madblock.api.utils.TimeUtils;
...
public class IP {
    ...
    public void setCurrentServer(GameServer server) {
        this.currentServer = server == null ? "null" : server.getName();

        Database.get().update(Table.IPS, new Set[] { new Set(TableIPs.CURRENT_SERVER, currentServer) }, new Where(TableIPs.UUID, uuid.toString()));
    }
    ...
    private static IP fromDatabase(UUID uuid) {
        if (!Database.get().contains(Table.IPS, new Column[] { TableIPs.UUID }, new Where(TableIPs.UUID, uuid.toString())))
            return null;
