...
public class SurvivalPrismSolarShopGUI extends PrismSolarShopGUI {
    ...
    protected boolean onOpen(OMPlayer player) {
        ...
        add(3, 0, new ShopItem(omp, Currency.PRISMS, 200, new ItemBuilder(Material.STONE_HOE, 1, "§a§lSurvival Package").addFlag(ItemFlag.HIDE_ATTRIBUTES), "§9§l100 Claimblocks", "§2§l50 Credits") {
            @Override
        ...
        add(3, 4, new ShopItem(omp, Currency.SOLARS, 1000, new PlayerSkullBuilder(() -> omp.getName(true), 1, "§a§lYour Skull Package"), "§a§l1x " + omp.getName(true) + "'s Skull") {
            @Override
        ...
        if (!omp.warpSlotPrisms())
            add(3, 6, new ShopItem(omp, Currency.PRISMS, 30000, new ItemBuilder(Material.WRITABLE_BOOK, 1, Warp.COLOR.getChatColor() + "§lWarp Package"), "§9§lPrisms Warp Slot") {
                @Override
                public void give(OMPlayer player) {
                    SurvivalPlayer omp = (SurvivalPlayer) player;
            ...
            add(3, 6, new EmptyItemInstance(new ItemBuilder(Material.BOOK, 1, "§c§lWarp Package", "§7§o" + omp.lang("Gekocht", "Bought")).build()));

        add(3, 8, new ShopItem(omp, Currency.PRISMS, 2500, new ItemBuilder(Material.SPRUCE_DOOR, 1, "§6§lHome Package"), "§6§l1 Home") {
            @Override
            public void give(OMPlayer player) {
                SurvivalPlayer omp = (SurvivalPlayer) player;
                omp.addExtraHomes(1);
