...
public class NpcNms_1_8_R1 implements NpcNms {
    ...
    public float[] handleMovement(Entity rideable, float speed, float backMultiplier, float sideMultiplier, float walkHeight) {
        ...
        try {
            ReflectionUtils.getDeclaredMethod(net.minecraft.server.v1_8_R1.Entity.class, "setYawPitch", float.class, float.class).invoke(nmsRideable.yaw, nmsRideable.pitch);
        } catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
        }

        nmsRideable.aG = nmsRideable.aE = nmsRideable.yaw;

        nmsRideable.S = walkHeight;

        float sideMot = (((EntityLiving) nmsRideable.passenger).aX * 0.5F);
        float forMot = ((EntityLiving) nmsRideable.passenger).aY;
    ...
    public void handleJump(Entity rideable, float jumpHeight) {
        ...
        Field field = ReflectionUtils.getDeclaredField(EntityLiving.class, "aW");
